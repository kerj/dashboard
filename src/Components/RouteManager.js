...
export default function RouteManager(props) {
  ...
  const ROUTES = {
    ...
    //strava
    'STRAVA - TOP RIDES': <BarChart dataToGraph={chartData} title={'STRAVA'} labelOne={'MILES VS '} labelTwo={' CLIMBING'} colorOne={'rgb(255,45,29)'} colorTwo={'rgb(162,54,40)'} />,
  }
  ...
  function makeNewChartData() {
    ...
    let stravaProps = weeklyData.strava
    ...
    switch (routes.route) {
      ...
      case 'MOBILE OPERATING SYSTEMS':
        ...
        setChartData(newChartData)
        break;
      case 'STRAVA - TOP RIDES':
        stravaProps.map((c) => {
          console.log(c)
          return newChartData.push(c);
        }) 
        setChartData(newChartData);
      default:
        break;
